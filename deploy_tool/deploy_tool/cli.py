...
from .config import save_config, load_config, load_state
...
def init():
    ...
    while True:
        ...
        if not aws_access_key_id or len(aws_access_key_id) != 20:
            click.echo("Invalid AWS Access Key ID format. Should be 20 characters.", err=True)
            continue
            
        aws_secret_access_key = click.prompt("AWS Secret Access Key", hide_input=True)
        if not aws_secret_access_key or len(aws_secret_access_key) != 40:
            click.echo("Invalid AWS Secret Access Key format. Should be 40 characters.", err=True)
            continue
    ...
    if save_config(config):
        click.echo("AWS credentials saved securely!")
    else:
